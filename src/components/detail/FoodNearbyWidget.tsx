...
export default function FoodNearbyWidget({ market }: { market: Market }) {
  return (
    <section className="card p-6" aria-label="Nearby Food Places">
      ...
      <p className="text-xs text-[#6B7280] mb-4">Popular eateries near this market</p>
        ...
        {market.foodNearby.map((place, i) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center shadow-sm text-base">
                {i === 0 ? '🥇' : i === 1 ? '🥈' : '🍴'}
              </div>
              <div>
